...
class TransformerEncoder(EncoderBase):
    ...
    def forward(self, src, knl=None, lengths=None, knl_lengths=None):
        ...
        emb, knl_bank_tgt, knl_mask = self.knltransformer(tgt_knl, None)  # Document k+1
        emb, src_bank, src_mask = self.histransformer(src[100:, :, :], None) # Utterance k
        his_bank = None
        his_mask = None
        for h in history:
            u = h[0]   # Utterance k
            k = h[1]   # Document k
            emb, knl_bank, knl_mask = self.knltransformer(k, None)  # Attentive Encoder(Document K)
            knl_bank_input = knl_bank.transpose(0, 1).contiguous()
            emb, his_bank, his_mask = self.htransformer(u, his_bank, knl_bank_input, knl_mask, his_mask)  # Incremental Transformer
